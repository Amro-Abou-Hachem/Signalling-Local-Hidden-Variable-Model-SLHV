...
function calculate_visibility(p, α, β, strategies=nothing)
    ...
    This function takes as an input the probability distribution and the allowed
        signalling weights α,β and returns the visibility in this SLHV.
    p: Probability distribution for which the visibility is to be calculated, it must have the shape array(na,nb,nx,ny)
    α: The signalling weights allowed on Alice's side, it must have the shape array(na,nx,ny,ny)
    β: The signalling weights allowed on Bob's side, it must have the shape array(nb,ny,nx,nx)
    ...
    @constraint(model, signalling_alice[a=1:na, x=1:nx, y=1:ny, y′=1:ny],
        sum(q[λ] * abs((strategies[λ][1][x, y] == a ? 1 : 0) - (strategies[λ][1][x, y′] == a ? 1 : 0)) for λ ∈ 1:nstrat) <= α[a, x, y, y′])
    @constraint(model, signalling_bob[b=1:nb, y=1:ny, x=1:nx, x′=1:nx],
...
function calculate_visibility_dual(p, α, β, strategies=nothing)
    """
    This function takes as an input the probability distribution and the allowed
        signalling weights α,β and returns the visibility in this SLHV, and the coefficients 
